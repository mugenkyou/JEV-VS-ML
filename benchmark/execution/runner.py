"""Benchmark Runner: Pipeline orchestration for data prep, ML training, Jev calls, and manifests."""
...
from ..data.splits import make_holdout, make_split
from ..models.training import train_lane, finish_result, write_json
from ..jev.client import JevClient, get_key
from ..jev.prompts import make_payload, few_shot_ids
from ..jev.cache import digest
from .gpu import probe_gpus, run_isolated


def environment():
    """Capture environment details for the immutable run manifest."""
    versions = {}
...
def prepare_suite(root, cfg, pilot_root=None):
    """Download datasets, create deterministic holdout/split partitions, and record immutable manifest."""
    root = Path(root)
    root.mkdir(parents=True, exist_ok=True)
    sources = {
        p.name: hashlib.sha256(p.read_bytes()).hexdigest()
        for p in Path(__file__).resolve().parent.parent.rglob('*.py')
    }
...
def run_ml(root, cfg, gpu_ids=None):
    """Execute classical ML model search, refit, and evaluation across CPU or GPU workers."""
    if cfg.get('use_cuml'):
    ...
    if cfg.get('require_gpu') and not gpu_ids:
        raise RuntimeError(
            'No GPU passed both library probes. Enable Kaggle GPU and inspect probe errors above. '
    ...
    with parallel_config(backend='loky', inner_max_num_threads=cfg['threads']):
        results = Parallel(n_jobs=workers, verbose=10)(
            delayed(train_lane)(lane, root, cfg, gpu_ids[i] if gpu_ids else None)
...
def request_partition(client, df, ids, meta, examples, cfg, description):
    """Issue parallel Jev requests for a single dataset partition."""
    def one(idx):
    ...
    return dict(
        raw=np.array([r['prediction'] for r in responses]),
...
def run_jev(root, cfg, client=None):
    """Execute all zero-shot and few-shot Jev benchmark evaluations with caching and rate limiting."""
    root = Path(root)
    ...
    for dataset in cfg['datasets']:
        ...
        for seed in cfg['seeds']:
            ...
            for mode in cfg['jev_modes']:
                ...
                policy_responses = []
                if classes == 2:
                    policy, policy_responses = request_partition(
                        client, df, split['policy'], meta, examples, cfg,
                        f'{dataset} {seed} {mode}: policy'
                    )
                else:
                    policy = dict(raw=np.array([], dtype=int), score=np.array([]), probability=None, default_threshold=0.5)
                test, responses = request_partition(
                    client, df, split['test'], meta, examples, cfg,
                    f'{dataset} {seed} {mode}: test'
                )
                write_json(folder / (name + '.responses.json'), dict(policy=policy_responses, test=responses))
                result = finish_result(
                    name,
                    df.iloc[split['policy']].label.to_numpy(),
                    df.iloc[split['test']].label.to_numpy(),
                    policy, test, classes, cfg,
                    dict(
                        examples=examples,
                        requested_model=cfg['jev_model'],
                ...
                result.update(
                    ...
                    test_ids=split['test'],
                    test_labels=df.iloc[split['test']].label.tolist()
                )
                write_json(result_path, result)
    return dict(
        attempts=getattr(client, 'attempts', None),
        estimated_input_cost=getattr(client, 'estimated_cost', None)
    )
