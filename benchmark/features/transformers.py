"""Feature Extraction: Training-fitted, estimator-appropriate feature representations."""
import numpy as np
...
class Features:
    """Estimator-appropriate feature extractor supporting both text and tabular inputs."""
    ...
    def fit(self, x, y):
        if self.meta['kind'] == 'text':
            ...
            self.selector = SelectKBest(
                chi2, k=min(w.shape[1], self.cfg['selected_features'])
            ).fit(w, y)
            self.svd = TruncatedSVD(
                n_components=max(1, min(self.cfg['svd_components'], min(w.shape) - 1)),
            ...
            self.tab = ColumnTransformer([
                ...
                ('category', make_pipeline(SimpleImputer(strategy='most_frequent'), OneHotEncoder(handle_unknown='ignore', sparse_output=False)), self.cats)
            ], sparse_threshold=0).fit(x)
        return self
    ...
    def transform(self, x):
        if self.meta['kind'] == 'text':
            ...
            return dict(
                linear=hstack([word, self.char.transform(x.text)], format='csr'),
    ...
    def description(self, key):
        ...
        return {
            'linear': 'word+character TF-IDF',
            'word': 'word/bigram TF-IDF',
