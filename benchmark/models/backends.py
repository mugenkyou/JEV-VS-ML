"""Backend Routing: Explicit cuML vs. scikit-learn / CUDA dispatching."""
import numpy as np


def route(name, rep, model, weighted, text, classes, cfg, gpu_id):
    """Explicitly map model configurations to cuML GPU, CUDA native, or scikit-learn CPU backends."""
    ...
    if name == 'Logistic regression':
        ...
        model = RandomForestClassifier(
            **{k: params[k] for k in ['n_estimators', 'max_depth', 'min_samples_leaf', 'max_features', 'random_state']},
        ...
        model = SVC(
            C=params['C'], kernel='rbf', gamma='scale', cache_size=512,
            tol=params['tol'], output_type='numpy'
        )
    else:
...
def probe_cuml(gpu_id, cfg):
    ...
    from ..evaluation.decisions import outputs

    rng = np.random.default_rng(912)
    x = rng.normal(size=(120, 8)).astype(np.float32)
    rows = []
    with cp.cuda.Device(gpu_id):
        for name, text, classes in [
            ('Logistic regression', False, 2),
            ('Logistic regression', True, 3),
            ('Random forest', True, 3),
            ('k-NN', False, 3),
            ('SVM', False, 2)
        ]:
            ...
            for weighted in ([False, True] if name in ['Logistic regression', 'SVM'] else [False]):
                ...
                if out['probability'] is not None:
                    ...
                    assert np.allclose(out['probability'].sum(axis=1), 1, atol=0.01)
